...
* <a href="#lm-98590-white_check_mark">LM-98590 :white_check_mark:</a>
* <a href="#lm-104697-white_check_mark">LM-104697 :white_check_mark:</a>
* <a href="#lm-84256-white_check_mark">LM-84256 :white_check_mark:</a>
* <a href="#mob-2682-white_check_mark">MOB-2682 :white_check_mark:</a>

## LM-98590 :white_check_mark:
...
# Commit
svn commit -m "LM-104697 Named account custom fields can not be set without permission." web/mkt-3.0/app/controller/abm/namedAccount/CreateForm.js
```


## LM-84256 :white_check_mark:
...
Ext4.define('Mkt3.controller.abm.accountList.AccountListTree', {
  ...
  onAccountListGridNameClick: function(accountListId) {
    ...
    for (var i = 0; i < childNodes.length; i++) {
      ...
      if (currNode.data.id == accountListId) {
        accountListTree
          .items
...
# Commit
svn commit -m "LM-84256 Highlight a record in AccountListTree after selecting a record in AccountListGrid." web/mkt-3.0/app/controller/abm/accountList/AccountListTree.js
```

## MOB-2682 :white_check_mark:

* **URL:** https://resource.marketo.com/jira/browse/MOB-2682?filter=-1
* **Description:** Default trigger for In-App program doesn't have operators disabled if the subscription language is not English
* **Widgets:** /web/mkt-ext-2.0/widgets/DataFormPanel.js

If the name is `Mobile App` or `Action`, the field will be disabled in line 886:
```javascript
if (inAppDefaultTrigger) {
  this.getForm().items.each(function(item){
    if (item.dataIndex === 'op' 
      && (item.fmeta.dataRecord.data.displayName === 'Mobile App' 
      || item.fmeta.dataRecord.data.displayName === 'Action')) {
        item.disabled = true;
    }
  });
}
...
```bash
cd /Users/lqin/devdocker/mlm
svn diff web/mkt-ext-2.0/widgets/DataFormPanel.js > MOB-2682.patch
# Commit
svn commit -m "MOB-2682 Disable default fields for all languages." web/mkt-ext-2.0/widgets/DataFormPanel.js
```
